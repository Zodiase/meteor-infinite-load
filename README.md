...
// Client Side
Meteor.startup(function() {
  const collection = new Mongo.Collection('data');
   ...
   */
  const infiniCollection = InfiniLoad(collection, {
    /**
     * A unique identifier for this wrapped collection. Useful when you need to
     *   have two InfiniLoad collections of the same Mongo.Collection.
     * Note that this option has to match on both the client and server side.
  ...
  template.onCreated(function(template) {
    template = this;
     ...
     * Reactive.
     */
    let visibleDocumentCursor = infiniCollection.find(filter, options);

    /**
     * A shortcut to the original `collection.findOne`.
     * Reactive.
     */
    let visibleDocument = infiniCollection.findOne(filter, options);

    /**
     * Get the number of documents that have been loaded.
     * Reactive.
     */
    let loadedDocumentCount = infiniCollection.count();

    /**
     * Get the number of old documents that have not been loaded.
     * Reactive.
     */
    let moreToLoadDocumentCount = infiniCollection.countMore();

    /**
     * Get the number of new documents that are available to be loaded.
     * Reactive.
     */
    let newUnloadedDocumentCount = infiniCollection.countNew();

    /**
     * Get the number of all documents.
     * Reactive.
     */
    let totalDocumentCount = infiniCollection.countTotal();

    /**
     * Check if there are any more old documents to load.
     * Reactive.
     */
    let hasOldDocumentToLoad = infiniCollection.hasMore();

    /**
     * Check if there are any more new documents to load.
     * Reactive.
     */
    let hasNewDocumentToLoad = infiniCollection.hasNew();
